...
deps() {
    ...
    # Install needed packages for script & for SGX app
    sudo apt install -y make wget build-essential
}
...
# latest sdk link
   wget -O sgx_linux_x64_sdk_latest.bin https://download.01.org/intel-sgx/sgx-linux/2.19/distro/ubuntu20.04-server/sgx_linux_x64_sdk_2.19.100.3.bin

   # Make the driver and SDK installers executable
   chmod +x ./sgx_linux_*.bin

   # Install the SDK in /opt/intel/sgxsdk  with latest file
   (echo no; echo /opt/intel/) | ./sgx_linux_x64_sdk_latest.bin
...
install_sgx_driver(){
   ...
   wget -O sgx_linux_x64_driver_latest.bin https://download.01.org/intel-sgx/sgx-linux/2.19/distro/ubuntu20.04-server/sgx_linux_x64_driver_2.11.54c9c4c.bin

   # Install the driver
   sudo ./sgx_linux_x64_driver_latest.bin
